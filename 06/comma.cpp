...
int main()
{
    ...
    // Note that the comma operator had the lowest precedence, so these two lines have different behavior.
    int z = (x, y);
    z = x, y;       // here, the assignment (z=x) takes precedence. So z gets assigned x, and y is evaluated then discarded.

    // This is an example of how the comma operator can be used within for loops.
    // The for loop syntax is: for (init; cond; incr) { body }, where init, cond, and incr must each be a single expression.
    // So the comma operator allows us to update both i and j in a single expression.
