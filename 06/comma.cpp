...
int main()
{
    ...
    // Note that the comma operator had the lowest precedence,
    // so these two lines have different behavior.
    int z = (x, y);
    z = x, y;   // here, the assignment (z=x) takes precedence. So z gets assigned x, y is evaluated then discarded.
    ...
    // So the comma operator allows use to update both i and j in a single expression.
