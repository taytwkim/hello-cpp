 ...
 * The prefix operator is more performant and should be preferred unless we specifically want
 * postfix behavior.
 */
...
int main()
{
    ...
    // But if we use the increment operator on its own (without using it in an expression), then
    ...
    // Again, C++ does not guarantee that function parameters will be evaluated in order, so
    // something like below has an undefined result. In clang, probably 1+2. In gcc, probably 2+2.
