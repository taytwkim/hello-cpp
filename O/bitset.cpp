 ...
 * Bitsets
 *
 * In modern computer architecture, every object is byte-addressable, which means the min size of an
 * object is a byte. This means that even a boolean (which is 1 bit) takes up a whole byte, which is
 * fine in most cases but is technically a waste of 7 bits. So in some cases, it might be helpful to
 * pack bits into a byte and manipulate the individual bits. This is where we we can use
 * "std::bitset".
 *
 * But note that the size of the bitset is rounded up to the size of size_t (64 bits in a 64-bit
 * machine). So a bitset of x bits does not mean we are using exactly x bits.
 */
