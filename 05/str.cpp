...
/*
 * Note that std::string and std::string_view (C++17) are features that have been introduced to
 * replace the traditional C-style strings. While C-style strings are still available in C++, they
...
void example1()
{
    ...
    std::string color{};
    std::cin >> color;  // here, the color will capture "Doe" that was buffered before, not the
                        // color that we input.
...
void example2()
{
     ...
     * Let's say we type "John" and press ENTER. The newline character could be buffered and picked
     * up as the next input.
     */
