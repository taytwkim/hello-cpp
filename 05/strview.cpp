 ...
 *
 * The analogy is that we are "viewing" the string through a window, but does not own or can modify it.
 *
 * This could be a bit misleading, but it's really just a pointer to where the characters are stored (plus the string length).
 * Because std::string_view is basically a pointer + length, we can pass it around without making a copy of the full string.
 *
 * string_view is read-only in the sense that the compiler will not let us update the characters using string_view,
 * but the underlying character buffer may still be mutable and could be modified if we have some other handle to that same memory.
 * 
...
void example1()
{
    // when we declare a string, a C-style literal (the RHS of the equation) "Hello world" is copied into the memory, which is slow
...
void example2()
{
    ...
    // So for function parameters, string_view may be preferred if we are not going to modify the string in the function (e.g., convert to lowercase).
    printSV(s);
}
...
void example4()
{
    ...
    // If x is destroyed or updated, y will not be affected but z might have unexpected consequences.
    std::string y = x;
    std::string_view z = x;
