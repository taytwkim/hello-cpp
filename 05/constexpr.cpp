 ...
 * In some contexts, an expression is required be a constant expression, or the compiler will
 * complain.
 */
...
int main()
{
     ...
     * a const variable's value will be known at compile-time, and thus can be used in a constant
     * expression.
     *
     * In this case, we can use the constexpr keyword to define a variable which can be used in a
     * constant expression. The compiler will complain if the object's value is not known at
     * compile-time.
     */
     ...
     * A constexpr function must evaluate at compile-time when the constant expression it is part of
     * must evaluate at compile time. Otherwise, a constexpr function may be evaluated at either
     * compile-time or runtime. To be eligible for compile-time execution, all arguments must be
     * constant expressions.
     */
