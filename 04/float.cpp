...
int main()
{
    ...
    // This is because 5.0 is by default a double, so this line will try to convert a double to a
    // float. But since list initialization does not allow narrowing conversions, the conversion
    // will fail. But float c = 5.0 will be fine.
    ...
    std::cout << 5.0 << '\n';  // will just print 5 (by default, std::cout will not print the
                               // fractional part of a number if the fractional part is 0.)
    std::cout << 6.7f << '\n';
